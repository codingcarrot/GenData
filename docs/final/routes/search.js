...
//SNPs db connection
const con = db_con.con(db_con.SNP_db);
...
function renSearchWithMessage(_message, req, res){
	con.query('select distinct(Name) from cellline;', function (err, results, fields)	{
		if (err) {throw err;
			console.log(err);}
...
router.post('/results', ensureAuthenticated, function(req,res){
		...
		// Search by ID (given ID, find the corresponding chrom, query the right table)
			...
			//for single ID
				...
				query = "select ID, CHROM, POS from lookup where ID in (" + con.escape(id) + ");";
